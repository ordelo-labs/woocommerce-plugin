...
class Integ_Admin
{
    ...
    */
    public function add_token_input($settings)
    {
     ...
     /*
    |--------------------------------------------------------------------------
    | Sync products attribute and categories
    |--------------------------------------------------------------------------
    |
    | This function hooks into the action of saving the request token,
    | it's mainly used to sync the product attributes and
    | categories, to avoid extra work our API we'll
    | send all product and categories so we
    | can map it latter.
    |
    */
    public function sync_attributes()
    {
        ...
        $this->client->categories()->upsert($category_names);
        // $this->client->attributes()->upsert($attribute_names);
    }
